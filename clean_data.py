...
class CleanData(object):
    ...
    def countByType(self):
        ...
        """
        count_list = []
        types = self.old_df.loc[:, 'type'].value_counts()
        print('\n-- 数据库中共有%d次列车。' % self.old_df.shape[0])
    ...
    def countByStartStation(self):
        ...
        :return: list: [(车站名称, 值)]
        """
        start_s = self.old_df.loc[:, 'start_s'].value_counts()
        start_s_list = []
    ...
    def countByEndStation(self):
        ...
        :return: list: [(车站名称, 值)]
        """
        end_s = self.old_df.loc[:, 'end_s'].value_counts()
        end_s_list = []
    ...
    def countByStation(self):
        ...
        :return: list: [(车站名称, 值)]
        """
        station = self.old_df.key.tolist()
    ...
    def countBySpeed(self):
        ...
        for n in self.type_dict:
            ...
            speed_n_s = speed_n.speed.tolist()
            n_speed = sum(speed_n_s)/len(speed_n_s)
    ...
    def sortDistance(self):
        ...
        :return: df:包含key_n列
        """
        distance = self.old_df[['code', 'type', 'start_s', 'end_s', 'km', 'period', 'key']].copy()
...
def main():
    ...
    clean_data.countByType()
    clean_data.countByStartStation()
    clean_data.countByEndStation()
    clean_data.countByStation()
    clean_data.countByDistance()
    clean_data.countByTopDistance()
    clean_data.countByTypeDistance()
    clean_data.countByPeriod()
    clean_data.countByTopPeriod()
    clean_data.countBySpeed()
