...
"""
车次库信息用的是12306的车次库（2019-11-12），有些老旧，某些车次不准确，但不影响总体的情况分析
在oklx中下载所有的车次信息（12306太慢了），由于需要不断调试，因此设置了日志文件
每次可以选择输入下载的数量，全部下载完毕后则不能下载，可以清空或设置log文件后重新下载
同时，对不能下载的车次，暂时保存至日志文件中，可以用patch函数再次尝试，如果还是不能下载，则保存到invalid文件中
"""
...
class GetTrainInfo(object):
    ...
    def getHTML(self, url, code):
        try:
            ...
            reps = reps.text.encode("latin1").decode("gbk")
...
def main():
    with open(r'data\logging.log', 'r', encoding='utf-8') as f:
        lines = f.readlines()
    ...
    get_train_info = GetTrainInfo(this_num, last_sum)
